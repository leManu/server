...
class MoveUsers extends Base {
	...
	protected function configure(): void {
		parent::configure();
		$this
			->setName('files:object:multi:move-users')
			->setDescription('Move all users from one object store configuration to another')
			->addArgument('source', InputArgument::REQUIRED, 'Object store configuration to move users from')
			->addArgument('target', InputArgument::REQUIRED, 'Object store configuration to move users to');
	}
	...
	public function execute(InputInterface $input, OutputInterface $output): int {
		...
		if (!isset($configs[$target])) {
			$output->writeln('<comment>Target object store configuration ' . $source . ' doesn\'t exist yet.</comment>');
			...
			$output->writeln('If these requirements are not met, you can manually create the target object store configuration in each node\'s configuration.');
			$output->writeln('');
